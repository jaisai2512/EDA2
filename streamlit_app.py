...
Questions: Create goals that investigate potential relationships between the variabels in the dataset , When creating the goals consider description , sample elements and type of measurement , Generate only  MULTIVARIATE ANALYSIS goals , For now DON'T CREATE GOALS which Involves Time(year,month,date)
Suggested Visualizations: Suggest a single and most effective visualization for answer the question , Consider the type of measurement and description for suggesting the visualization and sample elements ,follow these rules when needed:\ni) Continuous vs Continuous:Use Scatter Plot, Hexbin Plot, Density Plot, Contour Plot, Correlation Heatmap.\nii) Discrete vs Discrete:Use Clustered Bar Plot, Stacked Bar Plot, Mosaic Plot, Contingency Plot, Heatmap (for frequencies).\niii) Discrete vs Continuous:Use Violin Plot, Box Plot, Strip Plot, Swarm Plot, Bar Plot with Error Bars.iv) Categorical vs Categorical:\nUse Contingency Plot, Clustered Bar Plot, Mosaic Plot, Stacked Bar Plot, Heatmap (for frequencies).
