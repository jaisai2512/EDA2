...
Questions: Create goals that investigate potential relationships between the variabels in the dataset , When creating the goals consider description , sample elements and type of measurement , Generate only correlation analysis goals 
Suggested Visualizations: Suggest a single and most effective visualization for answer the question , Consider the type of measurement and description for suggesting the visualization and sample elemenst , follow these rules : continuous vs continuous use scatter , discrete vs discrete use Grouped Bar Chart
...
Your task is to improve and complete the plot_and_save(temp) function by:
    ...
    vii) Returning the plot as a buffer using:
        ...
        return buf.
 Considerations:
 i)When dealing with age bin them
Output only code.'''
