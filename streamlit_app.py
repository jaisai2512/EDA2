...
"""
    ...
    st.write(p_data)
    Q_system_prompt = f'''You are a highly skilled data analyst. Your task is to evaluate the provided goals. If a goal is not appropriate, please propose a new one by replacing the old one , the new goal can either be an improved version of the previous one or a completely new goal that replaces the old one , make sure do not produce duplicate goals.
