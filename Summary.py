...
When classifying, prioritize the semantic type and description to understand the data's purpose. For example, numeric values representing categorical or countable data like area codes should be classified as discrete, not continuous'''
  ...
  if(count == 2):
    st.write("reupload")
    exit()'''
  system_prompt ='''You are tasked with summarizing a json using the following template. For each field in the dataset, provide a summary covering all its key properties, such as the field name, description, semantic type, type of measurement, null counts, and sample elements. Ensure the structure follows the template below:\n{temp}
  '''
  temp = '''
